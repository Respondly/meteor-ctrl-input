    ...
    MediumEditor.prototype = {
        ...
        bindPaste: function () {
            ...
            this.pasteWrapper = function (e) {
                var paragraphs,
                    clipboardData = (self.isIE ? window.clipboardData : e.clipboardData),
                    text = '',
                    html = '',
                    p;
                ...
                if (clipboardData && clipboardData.getData && !e.defaultPrevented) {
                    e.preventDefault();

                    if (!self.isIE && self.options.cleanPastedHTML && clipboardData.getData('text/html')) {
                        return self.cleanPaste(clipboardData.getData('text/html'));
                    }
                    if (!self.options.disableReturn) {
                        ...
                        for (p = 0; p < paragraphs.length; p += 1) {
                            if (paragraphs[p] !== '') {
                                ...
                                if (clipboardData.items && clipboardData.items.length > 0) {
                                    hasFiles = clipboardData.items[0].type.has('image')
                                }
                                if (!hasFiles) {
                                    html += '<p><br></p>'
                                }
                        ...
                        document.execCommand('insertHTML', false, clipboardData.getData('text/plain'));
                    }
                }
            };
            for (i = 0; i < this.elements.length; i += 1) {
